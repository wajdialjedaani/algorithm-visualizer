...
function darkMode() {
  ...
  var btnOutline = []
    .concat(...document.querySelectorAll(".btnResize"))
  ...
  var cellSizeInput = document.querySelector("#cellSizeInput");

  cellSizeInput?.classList?.toggle("cellSizeInput-dark");
...
function sortingGo() {
  // animation if sorting go is selected
  menuChange();
...
function sortingBack() {
  // animation if sorting back button is selected
  menuChange();
...
window.addEventListener("load", function () {
  if (!Cookies.get("darkMode")) {
    //If no cookie, default to light mode
    Cookies.set("darkMode", "0", { expires: 999, sameSite: "strict" });
  } else if (Cookies.get("darkMode") === "1") {
    //Cookie says dark mode
...
document
  .querySelector("#fontIncrease")
  ?.addEventListener("click", increaseFontSize);
document
  .querySelector("#fontDecrease")
...
document
  .querySelector("#pathfindingGo")
  ?.addEventListener("click", pathfindingGo);
document.querySelector("#sortingBack")?.addEventListener("click", sortingBack);
document
  .querySelector("#searchingBack")
  ?.addEventListener("click", searchingBack);
document
  .querySelector("#pathfindingBack")
  ?.addEventListener("click", pathfindingBack);
